...
pub fn get_stokes_list(position: (f64, f64), velocity: (f64, f64), radius: f64, delta_t: f64)
    ...
    // Eigentlicher loop
    loop {
        // Erst neue Position, dann Geschwindigkeit, falls g oder VISC später ortsabhängig sein soll!
        let newrx = rx + vx * dt;
        let newry = ry + vy * dt;
        let newvx = vx + (gx - 6.0 * PI * radius * VISC * 0.000001 * vx) * dt;
        let newvy = vy + (gy - 6.0 * PI * radius * VISC * 0.000001 * vy) * dt;
